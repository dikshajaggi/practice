...
const Main = () => {
    ...
    async function getRest() {
        await axios.get("https://www.swiggy.com/dapi/restaurants/list/v5?lat=28.7040592&lng=77.10249019999999&offset=15&sortBy=RELEVANCE&pageType=SEE_ALL&page_type=DESKTOP_SEE_ALL_LISTING").then((data) => {
            ...
            setFilteredData(data?.data?.data?.cards)
            console.log(data?.data.data.cards, "cards api")
        })
    }
    ...
    const search = () => {
        setFilteredData(restData.filter((item) => item.data.data.name.toLowerCase().includes(searchvalue.toLowerCase())))
    }
    return (
        <div className='main-wrapper'>
            ...
            <div className='card-wrapper'>
                ...
                {filteredData.map((item) => {
                    const dataArr = item.data.data
                    console.log(dataArr, "item check", item.data.data.sla.restaurantId, "rest_id")
